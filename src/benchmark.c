...
int main( int argc, char **argv )
{
    ...
    for ( int m = 64; m < 10000; m = m+1+m/3 )
    {
        ...
        /* measure Gflop/s rate; time a sufficiently
         * long sequence of calls to eliminate noise */
        ...
        /* Subtract A*B from C using standard sgemm
         * (note that this should be 0 to within machine roundoff) */
        cblas_sgemm( CblasColMajor, CblasNoTrans, CblasTrans,
                m, m, n, -1, A, m, A, m, 1, C, m );
        ...
        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasTrans,
                m, m, n, -3.0*FLT_EPSILON*n, A, m, A, m, 1, C, m );

        /* After this test if any element in C is still positive
         * something went wrong in square_sgemm */
        for ( int i = 0; i < m * m; i++ )
            if ( C[i] > 0 )
            {
                printf("FAILURE: error in matrix multiply\
